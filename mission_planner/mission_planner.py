...
from .nav_aid import turn_command
import time
import cflib.crtp
...
class MissionPlanner(Node):
    ...
    def create_callback(self, drone,drones, channel):
        ...
        def listener_callback(msg):
            if msg.transforms:
                ...
                elif detectedTag in self.navigation: #navigation aid
                    self.get_logger().info('Target "%s" has been detected' % detectedTag)    
                    self.get_logger().info(f'Turning {drone} left 180 degrees on {detectedTag}')
                    turn_command(channel, int(drone[2:],16))                   
    ...
    def __init__(self):
        ...
        self.undetectedTags = {"tag36h11:32", "tag36h11:33", "tag36h11:30"}
        
        self.navigation = {"tag36h11:52", "tag36h11:53"}
